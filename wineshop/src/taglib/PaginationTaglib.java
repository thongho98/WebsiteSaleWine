...
public class PaginationTaglib extends SimpleTagSupport {
	...
	public void doTag() throws JspException {
		...
		try {
			out.write("<nav class=\"container col-md-4\">");
			...
			for (int itr = 0; itr < count; itr += steps) {
				if (offset == itr)
					out.write(constructLink((itr / 6 + 1) - 1 * steps, String.valueOf(itr / 6 + 1), "active", true));
				else
					out.write(constructLink(itr / 6 * steps, String.valueOf(itr / 6 + 1), null, false));
			}
